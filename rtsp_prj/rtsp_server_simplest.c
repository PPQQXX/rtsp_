...
// 传输h264或aac，二选一
#define SUPPORT_RTP_AAC 0   
//#define SUPPORT_RTP_H264 0

/*
 * 直接用RTP传aac时没有问题，即rtp_aac正常。
 * 
...
#if SUPPORT_RTP_AAC
            if (rtp_play_aac(serverRtpSockfd, client, AAC_FILE) < 0) {
#else
            if (rtp_play_h264(serverRtpSockfd, client, H264_FILE) < 0) {
#endif
...
int rtsp_talk_with_client(int server_sockfd) {
    ...
    while(1) {
        ...
        do_client(client_sockfd, &client, rtp_fd, rtcp_fd);
